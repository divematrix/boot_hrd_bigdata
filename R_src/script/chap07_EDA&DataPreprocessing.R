...
dataset <- read.csv("data/dataset.csv", header = T) # 헤더가 있는 경우
dataset
...
x <- dataset$price # price vector 생성
head(x)
dataset$price3 <- ifelse(!is.na(x), x, round(mean(x,na.rm=T),2)) # 평균으로 대체 #round() 평균
View(dataset)
sum(dataset$price2) # 2362.9
...
getwd()
setwd("C:/workspace/class_bigdata_R")
new_data <- read.csv("data/new_data.csv", header = T)
View(new_data)
...
write.csv(user_pay_data, "output/cleanData.csv", quote = F, row.names = F)

data <- read.csv("output/cleanData.csv", header = T)
View(data)
