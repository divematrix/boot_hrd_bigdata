...
--[2] 뷰(view) 정의하기
--		. 뷰를 생성하기 위해서는 create view 로 시작함
...
SQL>show user -- "system"유저
SQL>grant create view to scott; -- scott에게 create view 권한 부여하다
...
--	[문제]
--	기본 테이블은 emp_copy로 합니다.
--	20번 부서에 소속된 사원들의
--	사번과 이름, 부서번호, 상관의 사번을 출력하기 위한 select문을
--	emp_view20 이라는 이름의 뷰로 정의해 보세요.
...
--	주의)
select * from emp_view_copy where deptno = 30; -- failed
...
-- (3) -- inline view : 메인 쿼리문에 테이블 이름이 와야될 자리에 다시 select문이 온것
...
--          사원을 출력을 해보세요.
select	rownum, rnum, hiredate, empno, ename
from	(	select rownum rnum, hiredate, empno, ename, sal
			from	(	select *
						from emp
